...
function useTemplate(templateID, init) {
  ...
  return {
    ...
    insertTo: (target, before = null) => select(target).insertBefore(clone(), select(before)),
...
function useBinds(target, mutate) {
  ...
  const bindMap = Array.from(parent.querySelectorAll("[bind]")).reduce((obj, elem) => {
    elem.contenteditable = "true"
    elem.getAttribute('bind').match(/([\w:]+)/g).forEach(attr => {
      let [ prop, name, eventName ] = attr.split(':')
      if (!name) name = prop

      if (!!eventName) elem.addEventListener(eventName, e => res[name] = elem[prop])
 ...
 * @return {{value: *}} An object with a value property, triggering the function on change.
 */
